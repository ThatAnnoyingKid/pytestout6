...
def complete_lab(client, user_id, default_version_id, resource_id, resource):
    ...
    submit_result_request = pytestout6.SubmitResultRequest(
        ...
        points_scored = random.randrange(8, 11),
        points_possible = 10,
        passed = True,
...
def complete_quiz(client, user_id, default_version_id, resource_id, resource):
    create_exam_response = client.create_exam(
        user_id, 
        default_version_id,
        resource_id
...
def complete_all(client, login_response):
    ...
    for product in classes_response.get_activated_products():    
        ...
        for (resource, resource_map_entry) in assments:
            ...
            if resource.get_index().split(".")[0] == chapter and resource_map_entry.is_sim() and labCount < labs:
                complete_lab(client, user_id, default_version_id, resource_id, resource)
                labCount += 1
        ...
        for (resource, resource_map_entry) in assments:
            ...
            if resource.get_index().split(".")[0] == chapter and resource_map_entry.is_exam() and quizCount < quiz:
                complete_quiz(client, user_id, default_version_id, resource_id, resource)            
                quizCount += 1
...
def main():   
    ...
    while not should_exit:
        complete_all(client, login_response)
        keeps = input("Do you want to continue? \n A. Yes \n B. No \n")
        if keeps.lower() == "b":
            should_exit = True
